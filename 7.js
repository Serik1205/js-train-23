...
class EmployeeIterator {
	...
	// Метод, що перевіряє, чи є наступний елемент для ітерації
	#hasNext() {
		return this.#currentIndex < this.#employees.length;
	}

	// Метод, що повертає наступного працівника і збільшує індекс
	next() {
		if (this.#hasNext()) {
			...
			return null; // Немає наступного елемента
		}
	}

	// Метод для виведення імен всіх працівників в групі
	list() {
		const names = this.#employees.map(employee => employee.name);
		console.log(`Список працівників: ${names.join(', ')}`);
	}
}
